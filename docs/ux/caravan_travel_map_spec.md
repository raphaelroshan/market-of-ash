...
After a successful departure command, the regional map gives way to a side-on road view. The caravan crosses an authored corridor and stops at a readable midpoint observation. The player explicitly continues before an event or arrival is revealed, so the journey is never bypassed. Paid route costs have already been applied by the command result; animation duration never affects simulation.

Each corridor has a stable travel identity tied to its existing route ID:

- Old Road becomes the Ashen Milestones, with broken markers and ruined roadside masonry.
- Toll Road becomes the Warden Causeway, with maintained paving, lamps, and inspection-post language.
- Dry Cut becomes the Saltwind Cut, with exposed ridges and wind-carved navigation markers.

The header identifies leaving, the named road stop, an encounter, or the destination approach. Origin and destination silhouettes remain on opposite horizons, and the route's authored risk description stays visible. These are presentation profiles only; they do not add hidden encounters or alter the authoritative route forecast.
...
The settlement is a recurring hub rather than one long report. A stable bazaar directory exposes Trade, Jobs, Services / Intel, Crew, Outlook, and the Departure Gate. Trade is the default work surface. Choosing another stall replaces the trade ledger and transaction row instead of adding another layer of information to the screen. Each entry points to an existing authoritative action set:
...
The bazaar header keeps only the settlement identity, today's need, caravan resources, and compact regional pressure visible. Non-trade stalls use a settlement-tinted illustrated market strip with five keepers and a strongly marked active stall, while the actionable details remain in the adjacent rail. The illustration is presentation-only: keyboard, controller, pointer, and assistive-Web navigation continue through the real stall buttons. Longer explanations remain attached to the relevant stall or result.
