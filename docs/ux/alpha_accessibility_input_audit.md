...
4. CI structurally validates and headlessly launches the packaged x86-64 Windows PE, while run 205 rendered and validated the full 14-state minimum-window UI journey through Windows ANGLE. Run 210 additionally launched the packaged GUI visibly, verified `Market of Ash` and `0.9.0.0` in its PE resources, captured the exact 960×540 client area, and produced a visually inspected neutral Main Menu without desktop leakage. Physical clean-machine installation/distribution behavior and the documented OS-scaling matrix remain manual Windows gates.
