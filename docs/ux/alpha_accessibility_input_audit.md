...
1. CI run 195 completed the packaged Web matrix: 28 inspected captures cover 14 normal/Large text/event/result states at exact 960×540 and 1280×720 viewports, require matching app/ARIA state, and use decoded pixels to reject superficial transitions. A current-head local real-renderer matrix adds 42 inspected frames across 960×540, 1280×720, and 1920×1080 at Retina display scale 2.0. Remaining visual gates are Windows 125%, 150%, and 200% OS scaling plus current Chrome, Edge, and Firefox review.
...
4. CI run 195 structurally validated the packaged x86-64 Windows PE and launched it headlessly on a Windows runner. GUI launch on a clean Windows machine, installer/distribution behavior, visible resource-metadata inspection, and the documented OS-scaling matrix remain manual Windows gates.
