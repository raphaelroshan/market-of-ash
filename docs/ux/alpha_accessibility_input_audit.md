...
| Web assistive access | The Web export maintains a polite ARIA status region and descriptive canvas label for Main Menu, Shop, Departure, route-event, arrival, Pause, and destructive confirmation transitions. A visually hidden region becomes visible on focus and exposes current primary/safety-critical actions as real HTML buttons plus Shop/Departure planning as labeled select/number controls. Labels, options, values, integer bounds, enabled states, and linked descriptions mirror Godot; interaction routes through the existing Godot controls and handlers. | Headless UI tests verify state-specific announcements and control inventories, invalid input rejection, bounds clamping, and Shop/Departure synchronization. CI run 220 focuses and presses Enter on semantic buttons and changes the native planning fields throughout the 960×540 journey in Chrome, Firefox, and Edge while checking exact DOM/state parity; 1280×720 retains independent canvas-pointer coverage. Presentation/remapping controls and dynamic settlement actions remain canvas-only pending manual assistive-technology review. |
...
1. CI run 220 completed the current cross-engine packaged Web matrix: 84 captures cover 14 normal/Large text/event/result states at exact 960×540 and 1280×720 viewports in Chrome, Firefox, and Edge, require matching app/ARIA/action/form state, operate semantic actions and planning fields at the minimum viewport, and use decoded pixels to reject superficial transitions. A local real-renderer matrix adds 42 inspected macOS/OpenGL frames across 960×540, 1280×720, and 1920×1080 at Retina display scale 2.0; CI run 205 adds 14 inspected Windows/ANGLE frames at 960×540. Remaining visual gates are Windows 125%, 150%, and 200% OS scaling plus hands-on browser shortcut and assistive-technology review.
2. Controller behavior is configured and headlessly asserted but has not been exercised on physical Windows hardware.
3. Web builds announce screen changes and expose primary/safety-critical buttons plus Shop/Departure selectors and quantities through semantic HTML. Presentation/remapping controls and dynamic settlement actions do not yet have equivalent semantics, and the mirror is automation-verified rather than screen-reader-verified. Controller remapping is automated but still needs physical-device verification; essential information remains available as text, and interface cues are supplementary and can be disabled.
