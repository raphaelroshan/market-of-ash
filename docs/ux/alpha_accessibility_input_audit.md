...
| Text size/reflow | A persisted Main Menu option increases inherited and explicit interface text by 25%; launch controls remain above optional settings, long Shop context and Departure content scroll vertically, primary Buy/Sell/Plan actions remain pinned, and long dynamic action labels wrap across lines. | The full UI smoke path runs at 960×540 and checks visible primary actions at normal and large text, scaling without campaign mutation, settings-file round trip, scroll ancestors, and wrapped event controls. |
...
1. CI captures the packaged Web Main Menu and initial Settlement Shop at 960×540 and 1280×720 after confirming the loading overlay clears and the canvas accepts the Start action. The headless UI path also runs at 960×540 and asserts that Buy and Plan departure remain visible after enabling large text. Departure/event/arrival states, rendered large-text line wrapping, and high OS scaling still need human review.
