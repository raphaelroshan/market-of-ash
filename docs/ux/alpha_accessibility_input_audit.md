...
| Text size/reflow | A persisted Main Menu option increases inherited and explicit interface text by 25%; both Shop columns and the Departure action rail use vertical scrolling. | UI smoke test checks scaling without campaign mutation, settings-file round trip, and scroll ancestors around the primary trade and planning actions. |
| Color independence | Money, provisions, risk, crisis, standing, escalation, disabled reasons, and outcomes are always written as text; color is supplementary. Disabled route-event prerequisites remain persistently visible rather than tooltip-only. | Static UI inspection and UI text assertions. |
...
| Pause | A modal layer stops the scene tree, reports current run/save context, and offers Resume, Save, Load, and Return to main menu. Event decisions remain pending underneath it, and a failed protective autosave blocks return instead of discarding the live session. | UI smoke checks paused state, Resume focus, focus restoration, event preservation, and failed-autosave exit protection. |
| Feedback capture | Shop and Pause export a versioned JSON report with exact packaged commit/run, build, seed, state summary, contracts, events, route conditions, information, crew, ending context, commands, and game log; no personal identifiers are collected. Pause displays the result and local output path without closing. | UI smoke parses the report, verifies the reconstruction evidence, and checks visible Pause feedback while asserting no campaign mutation. |
