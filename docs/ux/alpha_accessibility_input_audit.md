...
| Text size/reflow | A persisted Main Menu option increases inherited and explicit interface text by 25%; both Shop columns and the Departure action rail use vertical scrolling, and long dynamic action labels wrap across lines. | UI smoke test checks scaling without campaign mutation, settings-file round trip, scroll ancestors, and wrapped event controls. |
