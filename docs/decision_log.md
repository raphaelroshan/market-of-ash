...
**Trade-off:** Input-remapping controls still rely on the Godot canvas. Browser automation verifies and operates the mirrored actions, planning fields, and presentation checkboxes through the minimum-viewport journey, but hands-on screen-reader testing remains required before claiming full control-level accessibility.
