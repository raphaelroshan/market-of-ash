...
**Progress:** M1–M4 are implemented. Desktop launch clamps the preferred 1600×900 window to the usable display, the 960×540 Main Menu and Introduction use an explicit stacked composition, and native captures assert visible bounds across Introduction, Bazaar, Departure, event, and arrival. The first Bazaar presents one ordinary-trade decision card before optional contracts. Bazaar, Departure, event-choice, and arrival/debrief copy comes from deterministic presenter modules while commands and world state retain all mutation. Departure now keeps every legal itinerary visible in a selectable two-column comparison board with fee, days, provisions, cargo opportunity, risk, confidence, and expected consequence. M5 journey-beat acceptance is next.
