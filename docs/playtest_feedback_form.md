...
Use **Export playtest report** from the Shop or Pause menu and attach the resulting JSON when practical; it contains no personal identifiers and records the build, platform, viewport, presentation settings, session duration, and observed time to first trade automatically.
