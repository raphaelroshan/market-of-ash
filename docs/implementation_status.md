...
**Investment update (2026-09-02):** MA-I1 through MA-I6 are complete for the automated scope. The guided opening proves a contract-free Water-and-Grain circuit and visible market response; the authored journey carries crew and optional side deals into the terminal economic/political receipt; Glasswind Reach closes a three-road economic triangle; and the versioned Windows candidate passes local package validation. Physical-device, storefront, assistive-technology, and moderated player testing remain external gates.

**GPT-5.6 packet update (2026-09-03):** MA-GPT56-1 through MA-GPT56-4 are complete for the repository-owned scope. The named clean-save vertical, presentation registry/view extraction, three complete regional sandboxes, deterministic 100-seed breadth matrix, and self-describing private-alpha package all pass. The current candidate is `0.16.0-early-access-rc2` with content `1.28.0`.

**Investment revalidation (2026-09-03):** The review-reopened MA-I1 shell gate and its dependent MA-I2 clean trade-flow gate are complete. Virtual-display clamping now uses the smaller physical/usable display bounds, compact columns fall back before their child minimum widths can collide, Introduction prose cannot widen its reading area, and every opening panel must preserve a 24 px safe area. Fresh 960×540, 1280×720, and 1600×900 real-renderer runs cover the opening, first purchase, route comparison, road consequence, arrival market change, return sale, optional pressure, and terminal receipt. The current candidate is `0.16.1-early-access-rc1` with content `1.28.0`.

**Playtest pacing tooling (2026-09-03):** Deliberate report export now includes a bounded, privacy-safe timeline of screen/context transitions and command outcomes. `tools/analyze_playtest_pacing.py` turns one report or a small cohort into milestone timing, dwell context, blocked-action, and incomplete-journey signals. Repo-local pacing and delivery-review skills constrain follow-up work to evidence-backed hypotheses; timing alone is never treated as proof of comprehension or fun.

**Player-copy pass (2026-09-04):** A repo-local writer skill now defines the restrained caravan-ledger voice, player-order audit, information hierarchy, regional vocabulary, and responsive verification contract. Its first pass removes QA and implementation language from the tutorial, road, encounter, arrival, recovery, and ending surfaces; event responses omit zero-value noise and separate exact costs from rewards. Mechanics, risks, IDs, and save semantics are unchanged. See `docs/writing_audit_2026-09-04.md`.

**Repeat-review closure (2026-09-04):** MA-GPT56-1A is complete. Startup fitting now normalizes an oversized window even when a virtual desktop reports it as maximized, then rechecks display bounds across four settled frames. Native evidence records and validates requested size, actual native-window size, logical viewport, and two-frame UI-state completion before every capture. Fresh Godot 4.4.1 evidence passes at 960×540, 1280×720, and 1600×900; the curated 1280×720 sequence proves Introduction 2 and the complete ordinary-trade-to-ending path. See `docs/ma_gpt56_1a_completion.md`.

**Journey-phase delivery pass (2026-09-04):** A delivery-review pass found that the right rail remained titled `DEPARTURE DESK` after commitment. It now names the actual beat—road movement, road stop, roadside decision, or destination arrival—while preserving the stable layout and authoritative travel state. Completion-aware captures assert the phase title, and fresh Godot 4.4.1 evidence passes at 960×540, 1280×720, and 1600×900. See `docs/delivery_review_2026-09-04.md`.

**Encounter-copy follow-up (2026-09-04):** Event cards now separate a concise authored action from its numeric terms. Non-zero costs and rewards appear once in dedicated rows, free choices omit the cost row, and historical command/debrief labels remain unchanged. Runtime validators protect the new optional `action_label` contract. See `docs/writing_audit_2026-09-04.md`.

**Road-endpoint identity pass (2026-09-04):** Road horizons now reuse each endpoint's data-driven settlement motif instead of one generic settlement block. Gate, reeds, brine, industrial, lantern, mirror, watchtower, quay, and peat silhouettes make origin and destination readable across all three regions without changing travel state or removing text labels. See `docs/delivery_review_2026-09-04.md`.

**Baseline branch:** `a0-command-result-boundary`  
**Baseline commit:** `5859d89` (`docs: add GPT agent alpha handoff roadmap`)  
**Roadmap status:** A0–A1 foundations, B0–B10's automated scope, the bounded Five-Well Basin deterministic vertical slice, **MA-EA-1 through MA-EA-6**, and **MA-I1 through MA-I6** are implemented and verified. The open-trade Feeds A–E remain intact: authored production/consumption roles, differentiated replenishment, ordinary-trade-first Bazaar language, non-contract balance gates, two causal replacement actors, reversible emergent-faction interaction, explicit cross-path reward balance, and two alternate endings reached through scenario failure followed by ordinary trade. Native evidence verifies the release-facing shell and all three playable regions at 960×540, 1280×720, 1600×900, and 1920×1080; every frame proves developer panels, diagnostics, and report actions are absent. Every numeric Early Access content floor and agent-implementable release-hardening gate is met. MA-I1 also reduces 25 deterministic world/save round trips from the audited approximately 5,161.91 ms to approximately 40–54 ms locally by eliminating whole-document copies from individual content lookups. Commercial art polish, physical-device and assistive-technology coverage, antivirus/storefront certification, and moderated player evidence remain external gates.
...
- Emberfen Refuge completes Siltfire as a three-market sandbox. Cloth, charcoal, spice, medicine, and water support ordinary trade on the high-exposure Emberfen Drift; an optional Smoke Cloth posting adds Bellkeeper value without gating profit.
- Ignoring or failing the smoke ward activates the Ash Sifters. Their replacement charcoal market and kiln action add local resilience and a persistent route improvement instead of silently restoring the official path.
...
- Saves declare save version 12 and runtime content version `1.28.0`; successful commands autosave through a temporary file with one backup generation, manual save/load summaries are visible, older saves migrate safely, and oversized files, malformed structure, invalid bounds/references, impossible pending journeys, forged current-content contract/event/scenario terms, or future versions cannot replace the active run.
...
3. **External release certification remains.** The project meets the floor for regions (3), settlements (11), goods (10), routes (9), crew (5), event families (9), standing factions (4), replacement actors (3), and endings (6), and MA-GPT56-4 automates the complete private-alpha package. Signing, antivirus reputation, storefront review, physical-device and assistive-technology checks, and moderated playtests cannot be claimed from repository automation.
...
- Desktop builds bootstrap at a display-safe 1280×720 and promote to the preferred 1600×900 when the physical and usable display bounds permit it, while preserving the 1280×720 logical canvas and 960×540 minimum window. Shop and Departure columns consume the available width, and the route board scales horizontally within safe marker bounds instead of leaving unused space.
...
- The current head cross-exports locally as a 64-bit Windows PE with a structurally verified embedded PCK using compatible x86-64 templates. Windows file/product versions use the numeric `0.16.1.0` required by PE resources while the player-facing project version is `0.16.1-early-access-rc1`.
...
- The agent-first QA wrapper now runs the authoritative verifier, defaults to the ordinary-trade semantic contract, and preserves an isolated release Main Menu capture plus environment and readiness metadata. A missing, editor-only, stale, or semantically invalid capture produces `INVALID_EVIDENCE` rather than a false pass; full scenario execution remains explicitly planned.
