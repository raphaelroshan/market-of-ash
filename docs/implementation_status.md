...
- The current head cross-exports locally as a 64-bit Windows PE with a structurally verified embedded PCK after installing the matching Godot 4.4.1 x86-64 templates. Windows file/product versions use the numeric `0.13.10.0` required by PE resources while the player-facing project version is `0.13.10-alpha-basin-vertical-slice`.
