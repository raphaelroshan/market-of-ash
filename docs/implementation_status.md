...
**GPT-5.6 packet update (2026-09-03):** MA-GPT56-1 through MA-GPT56-4 are complete for the repository-owned scope. The named clean-save vertical, presentation registry/view extraction, three complete regional sandboxes, deterministic 100-seed breadth matrix, and self-describing private-alpha package all pass. The current candidate is `0.16.0-early-access-rc1` with content `1.28.0`.
...
- The current head cross-exports locally as a 64-bit Windows PE with a structurally verified embedded PCK using compatible x86-64 templates. Windows file/product versions use the numeric `0.16.0.0` required by PE resources while the player-facing project version is `0.16.0-early-access-rc1`.
