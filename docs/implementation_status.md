...
2. **Local Chrome cannot start inside the host sandbox.** Matching Godot 4.4.1 Web templates are installed and repeated local exports produce the complete payload without recursively importing prior build outputs. Selenium still fails before session creation because Chrome's Mach-port bootstrap is denied, while Linux CI now supplies the authoritative packaged-browser evidence.
...
CI run 195 on implementation commit `c2e400e` passed repository policy, Ubuntu and Windows Godot tests, deterministic review, Windows/Web packaging and smoke checks, and the complete 28-frame packaged-browser matrix. Local results must still be recorded for every subsequent slice.
...
- Local preset parsing, project import, and current-head Web export pass with matching Godot 4.4.1 templates. CI exports the Windows executable and complete Web payload, verifies their required files, launches the packaged Windows game headlessly, and uploads a provenance manifest, source snapshot, and SHA-256 integrity list. Run 195's dependent Linux/Chrome job passed all 28 required captures: 14 normal/Large text/event/result states at exact 960×540 and 1280×720 viewports, with app state, ARIA announcements, trusted pointer traversal, non-mutating Return-to-Shop checks, and decoded-pixel transition validation. Local Selenium remains blocked before session creation by the host's Chrome Mach-port sandbox.
