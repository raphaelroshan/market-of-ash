...
**Roadmap status:** A0–A1 foundations, B0–B10's automated scope, and the bounded Five-Well Basin deterministic vertical-slice milestone are implemented and verified. The updated vertical-slice roadmap's open-trade Feeds A–E are implemented: authored production/consumption roles, differentiated replenishment, ordinary-trade-first Bazaar language, non-contract balance gates, a causal Well Commons response, reversible replacement-faction interaction, explicit cross-path reward balance, and an alternate ending reached through scenario failure followed by ordinary trade. The game-facing onboarding slice adds an illustrated introduction and a persistent two-journey tutorial through the real campaign. **The broader game-quality/private-alpha roadmap is not complete:** the current audit still finds horizontal clipping in the 1280×720 Main Menu and Introduction, and remaining work includes complete-flow presentation proof, responsive layout hardening, commercial art polish, device coverage, and moderated player evidence. These are follow-up gates, not reasons to reopen the completed deterministic slice.
...
- Responsive-shell M1 now clamps that preferred 1600×900 launch size to the current display's usable area, preventing the opening window from extending off-screen on a 1280×720 desktop. At the 960×540 minimum, Main Menu and Introduction deliberately stack illustration above interaction instead of only shrinking the split layout; Introduction copy scrolls independently while its navigation remains pinned. Native capture manifests now record and validate required panel/action bounds through Introduction, Bazaar, Departure, route event, and arrival/debrief, with an additional Introduction Large text frame.
...
- A local real-renderer Godot matrix now captures Main Menu, all three Introduction cards, Trade, the illustrated Job Board Bazaar, Pause, Departure, non-mutating return, the dedicated road view, Gatekeeper event/result, a deterministic realized-loss recovery result, destination Shop, and new-game confirmation, including Main Menu, Introduction, Shop, Departure, Pause, and event Large text coverage, at 960×540, 1280×720, 1600×900, and 1920×1080; the current macOS/OpenGL run covers all 104 frames. The validator accepts both Chrome RGB and Godot RGBA screenshots, checks exact dimensions/state/distinct transitions, validates required responsive-shell bounds, and verifies that the Departure map remains between its instructions and result text. These source-run images complement but do not replace packaged Windows/Web evidence.
...
- The current head cross-exports locally as a 64-bit Windows PE with a structurally verified embedded PCK after installing the matching Godot 4.4.1 x86-64 templates. Windows file/product versions use the numeric `0.13.2.0` required by PE resources while the player-facing project version is `0.13.2-alpha-basin-vertical-slice`.
