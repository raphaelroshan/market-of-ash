...
- Local preset parsing, project import, and current-head Web export pass with matching Godot 4.4.1 templates. CI exports the Windows executable and complete Web payload, verifies their required files, launches the packaged Windows game headlessly, and uploads a provenance manifest, source snapshot, and SHA-256 integrity list. Run 200 passed 28 required captures in each of Chrome, Firefox, and Edge: 14 normal/Large text/event/result states at exact 960×540 and 1280×720 viewports, with app state, ARIA announcements, trusted pointer traversal, non-mutating Return-to-Shop checks, and decoded-pixel transition validation. Local Selenium remains blocked before session creation by the host's Chrome Mach-port sandbox.
