...
| Web render | Launches the packaged Web build in Linux Chrome/Firefox and Windows Edge, verifies the loading overlay clears, drives semantic HTML actions by focus plus Enter at the minimum viewport and the real canvas by pointer at the standard viewport, validates the canvas label/live region/action region, and captures a declared normal journey, route-event, Pause, confirmation, and large-text matrix. | Blocks on browser startup, an unexpected app-published/assistive state, mismatched action labels/enabled states/descriptions, failed keyboard activation, superficial pixel changes, an incomplete per-viewport matrix, invalid PNGs, or unexpected dimensions. |
