...
| **Forecast calibration** | Forecast and resolution share the disclosed one-exposed-unit cargo-loss model, with deterministic regression coverage. | Preserve the invariant and rerun calibration after route, value, or incident changes. |
| **Automated simulation** | A read-only 100-seed policy harness and report exist under `tools/` and `research/playtest_simulation/`, including executable safety, strategy-variety, recovery, and preparation gates. | Rerun after any economy, event, route, contract, crisis, or onboarding change; do not balance by intuition alone. |
| **Roadmap completion** | B0–B10's automated alpha scope and open-trade Feeds A–E are implemented, including both relief and regulated/escort contracts, all four route events, three crew, two material faction paths, the Well Commons replacement path, bounded arms consequences, five endings, onboarding, accessibility settings, and Windows/Web release evidence. | Remaining alpha evidence is explicitly manual: moderated first-time-player comprehension, physical controller use, assistive technology, and Windows high-DPI scaling. |
