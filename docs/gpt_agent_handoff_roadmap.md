...
| **Roadmap completion** | B0–B10's automated alpha scope is implemented, including both relief and regulated/escort contracts, all four route events, three crew, two material faction paths, bounded arms consequences, four endings, onboarding, accessibility settings, and Windows/Web release evidence. | Remaining alpha evidence is explicitly manual: moderated first-time-player comprehension, physical controller use, assistive technology, and Windows high-DPI scaling. |
