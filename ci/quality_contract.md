...
The executable minimums live in `tests/test_game_quality.gd` and `tools/game_quality_metrics.gd`: the guided opening must complete across all 100 deterministic seeds with at least 60 ashmarks and 9 provisions remaining; at least three positive opening strategies across three goods and both road types must exist; authored staple, repair, medicine, and industrial trades must remain profitable without contracts; the best ordinary opening must retain at least 70% of the accessible relief contract's expected value; a ten-minute ordinary/contract/civic/faction fixture must track cash, provisions, hold, standing, time, and visit slots separately while keeping the contract between 100% and 120% of the best ordinary expected net; ignored relief must close causally, activate the Well Commons once, improve Reedwatch resilience, and create a profitable ordinary charcoal trade that was not viable beforehand; the active Commons must support two cooperation paths, one opposition/bypass path, and later reconciliation without closing ordinary trade; changing market/access state must rotate the best plan across three choices and both road types; a command-path cargo loss must recover starting cash within three outbound trades without reaching zero money or provisions; and the tutorial must reach its first cargo purchase within two authoritative commands. Any intentional threshold change requires updated evidence in `research/playtest_simulation/`.
