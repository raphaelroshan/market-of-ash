...
The repository contains a complete, replayable alpha vertical slice. Three Main Menu playtest paths expose the opening trade loop, conflict/recovery loop, and contract/crew loop without cheat state: each starts the same canonical campaign and changes only the selected first plan and guidance. Players can inspect explained prices, trade, accept a relief contract, recruit and assign crew, plan through a clickable regional map, resolve state-sensitive route conflicts with explicit threat, tactic, cost, risk, and expected-outcome summaries, then compare that plan with the authoritative resource, cargo, timing, risk-roll, and persistent-world result before entering the destination. A realized cargo loss names the best surviving local sale and lowest-risk affordable onward route. The latest comparison remains visible in the destination shop and is reconstructed from saved event history after loading. Players can influence factions and settlement resilience, steer a three-stage water crisis toward four distinct endings, and continue trading afterward. Desktop builds open at 1600×900 while retaining the established 1280×720 logical canvas and 960×540 minimum-window coverage. Versioned autosaves, backup recovery, Pause, accessibility preferences, deterministic diagnostics, and privacy-safe playtest reports support external testing.
...
From the Main Menu, choose **Guided Trade**, **Conflict & Recovery**, or **Contract & Crew**. These are reproducible entry points into the same campaign; they preselect the relevant cargo and route but do not grant resources, complete actions, or bypass normal validation.
