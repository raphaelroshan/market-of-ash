...
func _test_runtime_content() -> void:
	...
	_expect(MarketContent.ending_records().size() == 2 and MarketContent.ending("ending_warden_reserve").get("title", "") == "Order at the Cistern", "runtime content should expose both stable ending records")
