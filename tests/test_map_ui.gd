...
func _initialize() -> void:
	...
	_expect(menu_accessibility_actions.size() == 6 and menu_accessibility_actions[0].get("id") == "start_game" and menu_accessibility_actions[0].get("enabled") and menu_accessibility_actions[1].get("id") == "continue_game" and not menu_accessibility_actions[1].get("enabled"), "Web accessibility actions should expose enabled Start, disabled Continue, input remapping, and default restoration without inventing a second game rule")
	...
	_expect(menu_web_state.get("accessibility_order") == ["action:start_game", "action:continue_game", "control:reduce_motion", "control:large_text", "control:interface_sounds", "action:rebind_ui_accept", "action:rebind_ui_cancel", "action:rebind_ui_pause", "action:restore_default_bindings"], "Web accessibility controls should follow the Main Menu's mixed visual order")
	_expect(ui._web_accessibility_action_control("rebind_ui_pause") == ui.binding_buttons["ui_pause"] and ui._web_accessibility_action_control("restore_default_bindings") == ui.restore_bindings_button, "Web remapping actions should resolve to the existing Godot buttons")
	...
	_expect(ui.start_game_button.find_next_valid_focus() == ui.reduce_motion_checkbox, "Main Menu focus should move from Start to Reduce motion when Continue is disabled: %s" % ui.start_game_button.find_next_valid_focus())
