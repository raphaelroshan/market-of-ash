...
func _initialize() -> void:
	...
	_expect(int(report.get("report_version", 0)) == 6 and report.get("game_version", "") == "0.16.0-early-access-rc1" and report.get("build_commit", "") == "development" and int(report.get("seed", 0)) == 1107 and report.get("command_history", []).size() == 2, "playtest report should include schema, build, seed, and command evidence")
	...
	_expect(ui.event_threat_label.text == "THREAT · 25% MAX CARGO-LOSS ROLL" and ui.event_exposed_label.text.contains("1 MEDICINE") and ui.event_route_label.text.contains("ASHGATE → BRINE CROSS") and ui.event_decision_label.text.contains("Pay for certainty"), "event card should present threat, exposed cargo, road, and dilemma as a visible dossier")
	_expect(ui.event_rules_label.text.contains("No hidden health") and ui.event_readiness_label.text.contains("3/4 RESPONSES AVAILABLE"), "event card should expose its rules boundary and summarize how many responses are currently usable")
