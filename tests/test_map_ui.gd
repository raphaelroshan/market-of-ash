...
func _initialize() -> void:
	...
	_expect(ui.shop_decision_summary_label.text.contains("VALUE — buy 30 · sell 64 · road 4 · expected +14 ashmarks") and ui.shop_decision_summary_label.text.contains("ROAD / CAPACITY — Old Road · 1 provision · 35% cargo-loss risk"), "the first Bazaar should expose destination value and route burden in one summary")
	...
	_expect(ui.shop_market_preview_label.text.contains("SPREAD — buy 15 · sell 32 · +17 each · load total +34") and ui.shop_market_preview_label.text.contains("ROAD — 4 ashmarks · 1 provision · 35% cargo-loss risk"), "shop did not expose the selected ordinary-trade spread and route burden")
	...
	_expect(ui.bazaar_panel_title_label.text == "MARKET STALL" and ui.bazaar_section_label.text.begins_with("Choose a cargo") and not ui.bazaar_departure_hint_label.visible and not ui.opportunity_title_label.visible, "the trade rail should lead with one concise market purpose instead of repeating navigation and departure instructions")
	...
	_expect(ui.route_preview_label.text.contains("Held 0/2 selected Water") and ui.route_preview_label.text.contains("Buy 2 before departure") and ui.route_preview_label.text.contains("travel uses what is actually held"), "the pre-purchase forecast should distinguish its planned load from the empty caravan")
	...
	ui.world.money = 120
	ui._refresh_ui()
	_expect(ui.diagnostics_label.text.contains("build development") and ui.diagnostics_label.text.contains("seed 1107") and ui.diagnostics_label.text.contains("save v12") and ui.diagnostics_label.text.contains("content 1.28.0"), "shop diagnostics should expose reproducible build/seed/save/content versions")
	...
	ui._show_main_menu()
	_expect(ui.start_game_button.find_next_valid_focus() == ui.continue_game_button and ui.continue_game_button.find_next_valid_focus() == ui.settings_button, "enabling Continue after a save should insert it into the visible player-facing Main Menu focus cycle")
	...
	_expect(ui.route_preview_label.text.contains("Held 2/2 selected Water") and ui.route_preview_label.text.contains("planned load is covered"), "the route forecast should confirm when the selected load is present in the actual hold")
	ui.world.current_settlement = "reedwatch"
	...
	_expect(ui.get_viewport().gui_get_focus_owner() == ui.route_comparison_buttons[0], "opening departure planning should focus the selected itinerary card")
	...
	_expect(departure_accessibility_actions.size() == 5 and String(departure_accessibility_actions[0].get("id", "")).begins_with("route_plan_") and departure_accessibility_actions[3].get("id") == "return_to_shop" and departure_accessibility_actions[4].get("id") == "commit_departure", "Web accessibility actions should expose every route plan before reversible and committing Departure controls")
	...
	_expect(ui.cargo_quantity.get_line_edit().find_next_valid_focus() == ui.route_comparison_buttons[0] and ui.route_comparison_buttons.back().find_next_valid_focus() == ui.commit_departure_button and ui.commit_departure_button.find_next_valid_focus() == ui.return_to_shop_button, "Departure Tab order should traverse route comparisons before the pinned Commit and Return actions")
	...
	if ui.map_panel != null:
		...
		_expect(ui.map_panel._departure_dust_strength() > 0.9, "a newly committed animated departure should begin with a visible dust accent")
		...
		_expect(is_zero_approx(ui.map_panel._departure_dust_strength()), "departure dust should clear before the intermediate road stop")
	...
	_expect(ui.shop_market_preview_label.text.contains("recovers by 5% per day"), "consumer-market preview did not explain its faster recurring replenishment")
	ui._on_save_pressed()
	ui._on_start_game_pressed()
	ui._on_load_pressed()
	...
	_expect(ui.event_label.text.contains("ROAD SIGN — An Ash Warden clerk") and ui.caravan_context_label.text.contains("AHEAD — The Gatekeeper's Chalk"), "the road stop should reveal the approaching situation before asking the player to continue")
	ui._on_continue_journey_pressed()
	...
	_expect(ui.caravan_context_label.text.contains("NEXT — Choose") and ui.event_label.text.contains("FOCUSED RESPONSE"), "a pending route event should state the next action and reveal the focused response")
	...
	ui._on_map_settlement_selected("reedwatch")
	_expect(ui._selected_id(ui.destination_option) == committed_destination and ui.event_label.text.contains("FOCUSED RESPONSE"), "map clicks should not rewrite the committed destination or hide the focused response")
	...
	_expect(ui.event_stakes_label.text.contains("Highest cargo-loss chance: 25%") and ui.event_stakes_label.text.contains("Toll Road") and ui.event_stakes_label.text.contains("Medicine x1 worth 44 ashmarks") and ui.event_stakes_label.text.contains("Only the listed ashmarks"), "event card did not expose the threat model, route, cargo basis, and terms")
	_expect(ui.event_threat_label.text.contains("THREAT · UP TO 25% CARGO LOSS") and ui.event_threat_label.text.contains("MEDICINE x1") and not ui.event_exposed_label.visible and ui.event_route_label.text.contains("ASHGATE → BRINE CROSS") and ui.event_decision_label.text.contains("Pay for certainty"), "event card should compactly present threat, exposed cargo, road, and dilemma")
	_expect(ui.event_rules_label.text.contains("Only the costs and consequences written here") and not ui.event_rules_label.visible and ui.event_readiness_label.text.contains("3/4 RESPONSES READY"), "event card should preserve its terms boundary for detail and compactly summarize how many responses are usable")
	_expect(not ui.event_mode_label.visible and not ui.event_title_label.visible and not ui.event_setup_label.visible and ui.event_threat_label.is_visible_in_tree(), "the encounter rail should avoid repeating the title and setup before its visible threat dossier")
	...
	_expect(ui.event_choice_buttons[0].text.contains("PAY — Pay the posted toll") and ui.event_choice_buttons[0].text.contains("COST PAY 6 ASHMARKS") and ui.event_choice_buttons[0].text.contains("CARGO SAFE") and ui.event_choice_buttons[1].text.contains("25% CARGO RISK"), "event choices should compare action, exact cost, certainty, and risk without opening each response")
	_expect(ui.event_choice_buttons[0].autowrap_mode == TextServer.AUTOWRAP_WORD_SMART and ui.event_choice_buttons[0].custom_minimum_size.y <= 68.0 and ui.event_choice_list.columns == 2, "event choices should use a compact two-column comparison grid")
	...
	_expect(ui.event_choice_buttons[3].text.contains("LOCKED") and ui.event_choice_buttons[3].text.contains("Tess Oryn"), "disabled event prerequisites should remain readable on the compact card")
	...
	await process_frame
	await process_frame
	_expect(ui.event_label.text.contains("FOCUSED RESPONSE") and ui.event_label.text.contains("RESULT —"), "keyboard focus should reveal the complete authored response in the journey area")
	...
	_expect(normal_departure_scroll != null and ui.event_choice_buttons.all(func(choice): return choice.get_global_rect().position.y >= normal_departure_scroll.get_global_rect().position.y and choice.get_global_rect().end.y <= normal_departure_scroll.get_global_rect().end.y), "normal text should keep every route choice visible together inside the Departure scroll rail")
	...
	_expect(departure_scroll != null and ui.game_layer.get_global_rect().encloses(departure_scroll.get_global_rect()) and focused_event_choice == ui.event_choice_buttons[0] and ui.event_choice_buttons.all(func(choice): return choice.get_global_rect().position.y >= departure_scroll.get_global_rect().position.y and choice.get_global_rect().end.y <= departure_scroll.get_global_rect().end.y), "large text should keep the full encounter comparison visible inside the Departure scroll rail")
	...
	_expect(ui.journey_phase_title_label.text == "ROADSIDE DECISION" and ui.departure_status_label.text.contains("ROUTE DECISION"), "journey rail did not identify the paused roadside decision")
	ui._unhandled_input(cancel_event)
	...
	_expect(ui.journey_phase_title_label.text == "ARRIVED AT BRINE CROSS", "arrival should replace the planning-desk heading with the destination handoff")
	...
	_expect(ui.conflict_outcome_panel.visible and ui.conflict_outcome_label.text.contains("JOURNEY RESULT") and ui.conflict_outcome_label.text.contains("CHOICE — PAY / CERTAIN") and ui.conflict_outcome_label.text.contains("EXPECTED — -6 ashmarks") and ui.conflict_outcome_label.text.contains("ARRIVAL — -6 ashmarks") and ui.conflict_outcome_label.text.contains("The chosen terms held"), "resolved Gatekeeper choice should compare its stated expectation with the arrival")
	...
	_expect(ui.event_basis_label.visible and ui.event_basis_label.text.contains("REPAIR STOCK · 2 Scrap") and ui.event_route_label.text.contains("OLD ROAD"), "span event dossier should surface its frozen material and road basis")
	_expect(ui.event_choice_buttons.size() == 4, "span event should expose all four authored choices")
	_expect(ui.event_choice_buttons[0].text.contains("TRADE — Sell the repair stock") and ui.event_choice_buttons[0].text.contains("COST 2 REPAIR MATERIALS") and ui.event_choice_buttons[0].text.contains("GET 30 ASHMARKS") and ui.event_choice_buttons[0].text.count("30 ASHMARKS") == 1, "premium option did not separate its action, cargo cost, and exact reward")
	_expect(ui.event_choice_buttons[3].text.contains("RETURN"), "turn-back recovery did not disclose its movement result")
	...
	_expect(ui.event_basis_label.visible and ui.event_basis_label.text.contains("SHORTAGE · 2 WATER") and ui.event_basis_label.text.contains("+ 6 PREMIUM EACH"), "barrel event dossier should surface its frozen shortage and premium basis")
	_expect(ui.event_choice_buttons.size() == 4 and ui.event_choice_buttons[0].text.contains("GET ") and ui.event_choice_buttons[0].text.contains("ASHMARKS"), "barrel event did not expose all choices and the exact emergency payout")
	...
	_expect(ui.event_choice_buttons[2].text.contains("LOCKED") and ui.event_choice_buttons[2].text.contains("active water relief commitment"), "the unavailable contract response should expose its prerequisite on the compact card")
	...
	_expect(ui.event_stakes_label.text.contains("Medicine x1 worth") and ui.event_stakes_label.text.contains("Old Road"), "escort event did not expose its route and loss basis")
	_expect(ui.event_choice_buttons.size() == 4, "escort event should expose all four authored choices")
	_expect(ui.event_choice_buttons[0].text.contains("PAY — Hire the riders") and ui.event_choice_buttons[0].text.contains("COST PAY 10 ASHMARKS") and ui.event_choice_buttons[0].text.count("10 ASHMARKS") == 1 and ui.event_choice_buttons[1].text.contains("45% CARGO RISK") and ui.event_choice_buttons[1].text.contains("NO COST"), "escort choices did not separate action, payment, and solo-crossing risk")
	_expect(ui.event_choice_buttons[1].text.contains("MANEUVER —") and ui.event_stakes_label.text.contains("Highest cargo-loss chance: 45%"), "escort confrontation should expose its maximum threat and risky tactic consistently")
	...
	_expect(not ui.conflict_outcome_label.text.contains("EXPECTED — 0 ashmarks") and ui.conflict_outcome_label.text.contains("EXPECTED — +1 days") and ui.conflict_outcome_label.text.contains("information: three riders sponsor mark"), "waiting comparison should omit unchanged cash while showing its expected time cost and persistent information result")
	ui._on_enter_settlement_pressed()
	...
	_expect(ui.conflict_outcome_label.text.contains("CHOICE — MANEUVER / 45% RISK") and ui.conflict_outcome_label.text.contains("Medicine -1") and ui.conflict_outcome_label.text.contains("Risk realized: road roll 24 against 45"), "risky confrontation comparison should explain the stated risk, actual roll, and realized cargo loss")
	_expect(ui.conflict_outcome_label.text.contains("RECOVERY — Medicine x1 remains and would sell here for 52 ashmarks") and ui.conflict_outcome_label.text.contains("lowest-risk affordable onward route") and ui.conflict_outcome_label.text.contains("The caravan can continue"), "a realized cargo loss should name a concrete surviving sale and affordable onward route")
	_expect(ui.conflict_outcome_receipt.visible and ui.conflict_outcome_receipt_kicker.text == "RISK REALIZED" and ui.conflict_outcome_receipt_title.text == "Medicine x1 lost" and ui.conflict_outcome_receipt_detail.text.contains("Road roll 24") and ui.conflict_outcome_receipt_detail.text.contains("risk 45"), "a realized cargo loss should lead with a compact, redundant loss receipt")
	_expect(ui._web_accessibility_announcement().contains("RECOVERY — Medicine x1 remains") and ui._web_accessibility_announcement().contains("The caravan can continue"), "arrival accessibility announcement should include the concrete recovery path")
	...
	_expect(ui.ending_panel.visible and ui.ending_label.text.contains("CAMPAIGN DEBRIEF") and ui.ending_label.text.contains("Open Routes, Shared Wells") and ui.ending_label.text.contains("ROUTE TIMELINE") and ui.ending_label.text.contains("CARGO & CASH") and ui.ending_label.text.contains("REPLAY EXPERIMENT"), "completed relief should receive a complete campaign debrief")
	_expect(ui.ending_continue_button.visible and ui.ending_replay_button.visible and ui.ending_title_button.visible and ui.ending_feedback_button.visible, "campaign debrief should offer continue, replay, title, and report actions")
	var ending_action_ids: Array = ui._web_ui_state().get("accessibility_actions", []).map(func(action: Dictionary) -> String: return String(action.get("id", "")))
	_expect(ending_action_ids.has("ending_continue") and ending_action_ids.has("ending_replay") and ending_action_ids.has("ending_title") and ending_action_ids.has("ending_feedback"), "Web accessibility should expose every terminal debrief action")
	ui._on_ending_continue_pressed()
	_expect(not ui.ending_panel.visible and ui.world.ending_id == "open_routes_relief", "continuing after the debrief should preserve the concluded world while returning control to trade")
	ui._on_start_game_pressed()
	...
	_expect(ui.ending_panel.visible and ui.ending_label.text.contains("The Best Margin") and ui.ending_label.text.contains("Trade for relief and settlement resilience"), "profit-first ending should update the debrief with a concrete alternate experiment")
	...
	_expect(is_zero_approx(ui.map_panel._departure_dust_strength()), "reduced motion should suppress the departure dust animation")
	ui._show_trade_receipt("PURCHASE SEALED", "Water x1 loaded · 10 ashmarks paid · hold 1/12")
	_expect(ui.trade_receipt_panel.visible and ui.trade_receipt_panel.scale == Vector2.ONE and is_equal_approx(ui.trade_receipt_panel.modulate.a, 1.0), "reduced motion should retain the readable receipt while suppressing its scale and fade flourish")
	ui._finish_trade_receipt(ui.trade_receipt_generation)
	_expect(not ui.trade_receipt_panel.visible, "the receipt should leave no persistent overlay after its presentation window")
	ui._on_continue_journey_pressed()
