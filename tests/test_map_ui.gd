...
func _initialize() -> void:
	...
	_expect(ui.bazaar_navigation_buttons.size() == 5 and ui.bazaar_navigation_buttons[0].text == "Market Stall" and ui.bazaar_navigation_buttons[1].text == "Job Board" and ui.bazaar_navigation_buttons[2].text.contains("Intel"), "the bazaar should expose a stable directory for its major action groups")
	...
	_expect(ui.get_viewport().gui_get_focus_owner() == ui.crew_buttons[0], "the Crew stall should lead directly to the first available person")
	ui._on_bazaar_navigation_pressed("outlook")
	_expect(ui.campaign_outlook_label.visible and ui.bazaar_outlook_button.text.contains("Hide"), "the regional outlook should stay collapsed until requested")
	ui._on_bazaar_navigation_pressed("outlook")
	ui._on_bazaar_navigation_pressed("trade")
	_expect(not ui.campaign_outlook_label.visible and ui.get_viewport().gui_get_focus_owner() == ui.shop_good_option, "closing Outlook and returning to Trade should restore the focused market flow")
	...
	if ui.map_panel != null:
		...
		_expect(ui.map_panel.travel_phase == "road" and ui.continue_journey_button.visible and ui.get_viewport().gui_get_focus_owner() == ui.continue_journey_button, "the first travel leg should stop at a readable road view with an explicit continue action")
	...
	ui.map_panel._process(2.0)
	_expect(ui.map_panel.travel_phase == "road" and ui.continue_journey_button.visible and ui.status_label.text.contains("ON ROAD"), "the caravan should pause at the intermediate road view before revealing the event")
	ui._on_continue_journey_pressed()
