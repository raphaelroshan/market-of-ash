...
func _initialize() -> void:
	...
	_expect(ui._web_accessibility_announcement().contains("Start Guided Trade is focused") and ui._web_accessibility_announcement().contains("Conflict and Recovery") and ui._web_accessibility_announcement().contains("Contract and Crew") and ui._web_accessibility_announcement().contains("Enter / Space") and ui._web_accessibility_announcement().contains("A / Cross"), "Web accessibility fallback should announce every Main Menu playtest path and the active accept bindings")
	...
	_expect(float(logical_viewport.get("width", 0)) > 0.0 and float(logical_viewport.get("height", 0)) > 0.0 and not Dictionary(menu_targets.get("start_game", {})).is_empty() and not Dictionary(menu_targets.get("start_conflict", {})).is_empty() and not Dictionary(menu_targets.get("start_campaign", {})).is_empty() and not Dictionary(menu_targets.get("large_text", {})).is_empty(), "Web diagnostics should publish logical viewport and visible Main Menu target geometry")
	_expect(menu_accessibility_actions.size() == 8 and menu_accessibility_actions[0].get("id") == "start_game" and menu_accessibility_actions[0].get("enabled") and menu_accessibility_actions[1].get("id") == "start_conflict" and menu_accessibility_actions[2].get("id") == "start_campaign" and menu_accessibility_actions[3].get("id") == "continue_game" and not menu_accessibility_actions[3].get("enabled"), "Web accessibility actions should expose all three canonical start paths, disabled Continue, input remapping, and default restoration")
	...
	_expect(menu_web_state.get("accessibility_order") == ["action:start_game", "action:start_conflict", "action:start_campaign", "action:continue_game", "control:reduce_motion", "control:large_text", "control:interface_sounds", "action:rebind_ui_accept", "action:rebind_ui_cancel", "action:rebind_ui_pause", "action:restore_default_bindings"], "Web accessibility controls should follow the Main Menu's mixed visual order")
	_expect(ui._web_accessibility_action_control("start_conflict") == ui.start_conflict_button and ui._web_accessibility_action_control("start_campaign") == ui.start_campaign_button and ui._web_accessibility_action_control("rebind_ui_pause") == ui.binding_buttons["ui_pause"] and ui._web_accessibility_action_control("restore_default_bindings") == ui.restore_bindings_button, "Web start-path and remapping actions should resolve to the existing Godot buttons")
	_expect(menu_accessibility_controls.size() == 3 and menu_accessibility_controls[0].get("id") == "reduce_motion" and menu_accessibility_controls[1].get("id") == "large_text" and menu_accessibility_controls[2].get("id") == "interface_sounds", "Web accessibility controls should expose Main Menu presentation settings in visual order")
	...
	_expect(ui.start_game_button != null and ui.start_game_button.text.contains("Start Guided Trade"), "main menu should expose the guided trade path")
	_expect(ui.start_conflict_button != null and ui.start_conflict_button.text.contains("Start Conflict & Recovery"), "main menu should expose the conflict and recovery path")
	_expect(ui.start_campaign_button != null and ui.start_campaign_button.text.contains("Start Contract & Crew"), "main menu should expose the contract and crew path")
	_expect(ui.start_game_button.find_next_valid_focus() == ui.start_conflict_button and ui.start_conflict_button.find_next_valid_focus() == ui.start_campaign_button and ui.start_campaign_button.find_next_valid_focus() == ui.reduce_motion_checkbox, "Main Menu focus should traverse all start paths before settings when Continue is disabled")
	...
	for menu_control in [ui.start_game_button, ui.start_conflict_button, ui.start_campaign_button, ui.continue_game_button, ui.reduce_motion_checkbox, ui.large_text_checkbox, ui.interface_sounds_checkbox, ui.restore_bindings_button, ui.quit_button]:
	...
	_expect(Array(initial_web_state.get("accessibility_order", [])).slice(0, 3) == ["control:shop_good", "control:shop_quantity", "action:shop_buy"], "Shop semantic planning fields should precede its actions")
	...
	_expect(pause_accessibility_actions.size() == 5 and pause_accessibility_actions[0].get("id") == "pause_resume", "Web accessibility actions should expose the complete Pause action set with Resume first")
	...
	_expect(ui.start_game_button.find_next_valid_focus() == ui.start_conflict_button and ui.start_campaign_button.find_next_valid_focus() == ui.continue_game_button and ui.continue_game_button.find_next_valid_focus() == ui.reduce_motion_checkbox, "enabling Continue after a save should rebuild the Main Menu focus cycle around all start paths")
	_expect(ui.start_game_button.text.contains("New Guided Trade") and ui.start_conflict_button.text.contains("New Conflict & Recovery") and ui.start_campaign_button.text.contains("New Contract & Crew") and ui.start_game_button.tooltip_text.contains("Existing campaign files remain") and ui.start_conflict_button.tooltip_text.contains("Existing campaign files remain") and ui.start_campaign_button.tooltip_text.contains("Existing campaign files remain"), "a validated save should make the destructive fresh-start distinction visible on every playtest path before selection")
	ui._show_main_menu()
	ui._on_load_pressed()
	_expect(ui.active_playtest_path_id == "continued_campaign" and ui.playtest_status_label.text.contains("CONTINUED CAMPAIGN"), "Continue should resume saved state without claiming one of the fresh-run guidance paths")
	...
	_expect(ui.start_game_button.text.contains("New Guided Trade") and ui.start_conflict_button.text.contains("New Conflict & Recovery") and ui.start_campaign_button.text.contains("New Contract & Crew") and ui.start_game_button.tooltip_text.contains("Existing campaign files remain") and ui.start_conflict_button.tooltip_text.contains("Existing campaign files remain") and ui.start_campaign_button.tooltip_text.contains("Existing campaign files remain"), "unreadable save files should still require confirmation on every playtest path before a fresh run can overwrite them")
	ui._on_load_pressed()
	...
	_expect(int(report.get("report_version", 0)) == 6 and report.get("game_version", "") == "0.9.0-alpha-roadmap" and report.get("build_commit", "") == "development" and int(report.get("seed", 0)) == 1107 and report.get("command_history", []).size() == 2, "playtest report should include schema, build, seed, and command evidence")
	...
	_expect(ui.active_playtest_path_id == "conflict_recovery" and ui._web_ui_state().get("playtest_path_id", "") == "conflict_recovery" and ui.world.day == 1 and ui.world.money == 120 and int(ui.world.cargo.get("weight", 0)) == 0 and ui._selected_id(ui.shop_good_option) == "medicine" and int(ui.shop_quantity.value) == 2 and ui._selected_id(ui.destination_option) == "brine_cross" and ui._selected_id(ui.route_option) == "toll_road", "Conflict & Recovery should start the canonical campaign with only its route ingredients selected")
	_expect(ui.playtest_banner.text.contains("CONFLICT & RECOVERY") and ui.playtest_status_label.text.contains("STEP 1 OF 3") and ui.playtest_status_label.text.contains("Buy 2 Medicine"), "Conflict & Recovery should explain its first normal-game action")
	...
	_expect(ui.playtest_banner.text.contains("CONTRACT & CREW") and ui.playtest_status_label.text.contains("Accept Reedwatch Water Relief") and ui.playtest_status_label.text.contains("Recruit and assign one crew member"), "Contract & Crew should explain its first normal-game commitments")
	...
	ui._on_buy_pressed()
	_expect(ui.playtest_status_label.text.contains("STEP 2 OF 3") and ui.playtest_status_label.text.contains("Medicine is loaded"), "Conflict & Recovery should advance after its normal purchase")
	...
	_expect(ui.playtest_status_label.text.contains("STEP 3 OF 3") and ui.playtest_status_label.text.contains("Gatekeeper conflict is active"), "Conflict & Recovery should advance when its deterministic event is active")
	...
	_expect(ui.playtest_status_label.text.contains("RUN COMPLETE") and ui.playtest_status_label.text.contains("Gatekeeper conflict"), "Conflict & Recovery should complete after the route choice resolves")
