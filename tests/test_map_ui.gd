...
func _initialize() -> void:
	...
	_expect(ui.event_threat_label.text == "THREAT · UP TO 25% CARGO LOSS" and ui.event_exposed_label.text.contains("MEDICINE x1") and ui.event_route_label.text.contains("ASHGATE → BRINE CROSS") and ui.event_decision_label.text.contains("Pay for certainty"), "event card should present threat, exposed cargo, road, and dilemma as a visible dossier")
	_expect(ui.event_rules_label.text.contains("Only the costs and consequences written here") and ui.event_readiness_label.text.contains("3/4 RESPONSES AVAILABLE"), "event card should expose its terms boundary and summarize how many responses are currently usable")
	...
	_expect(ui.event_choice_buttons[0].text.contains("PAY / CERTAIN — Pay the posted toll") and ui.event_choice_buttons[0].text.contains("IF CHOSEN") and ui.event_choice_buttons[0].text.count("6 ashmarks") == 1 and ui.event_choice_buttons[1].text.contains("MANEUVER / RISK ROLL 25%"), "event choices should distinguish action, exact cost, certainty, and expected consequence without repeated terms")
	...
	_expect(ui.event_choice_buttons[0].text.contains("TRADE / CERTAIN — Sell the repair stock") and ui.event_choice_buttons[0].text.contains("COST — 2 repair materials") and ui.event_choice_buttons[0].text.contains("RECEIVE — 30 ashmarks") and ui.event_choice_buttons[0].text.count("30 ashmarks") == 1, "premium option did not separate its action, cargo cost, and exact reward")
	...
	_expect(ui.event_choice_buttons.size() == 4 and ui.event_choice_buttons[0].text.contains("RECEIVE —") and ui.event_choice_buttons[0].text.contains("ashmarks"), "barrel event did not expose all choices and the exact emergency payout")
	...
	_expect(ui.event_choice_buttons[0].text.contains("PAY / CERTAIN — Hire the riders") and ui.event_choice_buttons[0].text.contains("COST — pay 10 ashmarks") and ui.event_choice_buttons[0].text.count("10 ashmarks") == 1 and ui.event_choice_buttons[1].text.contains("45% cargo risk") and not ui.event_choice_buttons[1].text.contains("COST —"), "escort choices did not separate action, payment, and solo-crossing risk")
	_expect(ui.event_choice_buttons[1].text.contains("MANEUVER / RISK ROLL 45%") and ui.event_stakes_label.text.contains("Highest cargo-loss chance: 45%"), "escort confrontation should expose its maximum threat and risky tactic consistently")
