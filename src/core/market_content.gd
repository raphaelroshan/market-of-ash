...
const REQUIRED_SETTLEMENT_IDS := ["ashgate", "brine_cross", "cinderford", "hollow_market", "reedwatch", "sunfall_exchange", "kiln_rest", "mirror_wells", "mothlight_quay", "blackreed_post", "emberfen_refuge"]
const REQUIRED_ROUTE_IDS := ["old_road", "toll_road", "dry_cut", "glasswind_trace", "mirror_run", "emberglass_byway", "salt_causeway", "reedline_track", "emberfen_drift"]
	...
	if typeof(settlements_data) != TYPE_DICTIONARY:
		...
		for settlement_id in REQUIRED_SETTLEMENT_IDS:
			...
			if typeof(identity_value) != TYPE_DICTIONARY:
				...
				if String(identity.get("landmark", "")) not in ["gate", "brine", "forge", "lanterns", "reeds", "glass", "kiln", "mirrors", "quay", "watchtower", "peat_stacks"]:
