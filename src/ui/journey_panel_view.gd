...
static func snapshot(world, travel_phase: String, waypoint_label: String, arrival_pending: bool, committed_journey_message: String, last_outcome_text: String) -> Dictionary:
	...
	var state := "planning"
	var panel_title := "DEPARTURE DESK"
	...
	if actively_traveling:
		...
		state = "arrival"
		panel_title = "ARRIVED AT %s" % settlement_name.to_upper()
		...
		caravan_context = "JOURNEY COMPLETE\nNEXT — Review the result, then enter the Bazaar."
	return {
		"state": state,
