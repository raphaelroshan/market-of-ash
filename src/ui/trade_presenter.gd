...
static func route_preview_text(world, good_id: String, quantity: int, origin: Dictionary, destination: Dictionary, route: Dictionary, world_context: Dictionary) -> String:
	...
	if held_quantity < quantity:
		load_check = "LOAD CHECK — Held %d/%d selected %s. Buy %d before departure to carry the full load; travel uses what is actually held." % [held_quantity, quantity, good_id.capitalize(), quantity - held_quantity]
	else:
		load_check = "LOAD CHECK — Held %d/%d selected %s. The planned load is covered; departure carries everything in the hold." % [held_quantity, quantity, good_id.capitalize()]
	var faction_text := ""
