...
When the first Toll Road event is eligible, the simulator uses a deliberately simple resolution policy: pay the posted toll when six ashmarks remain, otherwise wait for stamped review. This keeps automated runs complete without claiming to model human preference. Event-choice balance still requires the dedicated truth table and human comprehension checks.
