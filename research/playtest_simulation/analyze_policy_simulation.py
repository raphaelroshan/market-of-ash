...
def main() -> int:
    ...
    arms_rows = pd.DataFrame(data.get("arms_policy_rows", []))
    if not arms_rows.empty:
        arms_summary = (
            arms_rows.groupby("policy", sort=False)
            .agg(
                runs=("seed", "count"),
                completion_rate=("status", lambda values: (values == "completed").mean()),
                median_economic_profit=("economic_profit", "median"),
                mean_economic_profit=("economic_profit", "mean"),
                mean_escalation=("arms_escalation", "mean"),
            )
            .reset_index()
        )
        arms_summary.to_csv(output / "arms_policy_summary.csv", index=False)
        arms_display = arms_summary.copy()
        arms_display["completion_rate"] = arms_display["completion_rate"].map(pct)
        arms_display.columns = ["Policy", "Runs", "Completed", "Median economic", "Mean economic", "Mean escalation"]
        arms_table = markdown_table(arms_display)
    else:
        arms_summary = pd.DataFrame()
        arms_table = "No arms-policy probe was recorded."
